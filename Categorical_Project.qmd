...
We aim to understand how the neural network works and to implement it on simulated data. We then compare the result with the one generated by the logistic regression model. At the end we want to compare the performance between logistic regression and NN in this classical example.

As we previously mentioned, an ANN mainly consists of three major components: the inputs, the weights and biases, and the activation function. Together they form a neuron that takes the input information, process it and returns an output based on the activation as threshold. The input forms the first layer of our neural network, often referred as the input layer. Similarly, the output layer consists of neurons that generate outputs. Any additional layers between the input and output layer are referred as the hidden layers. The input data is sent to the neurons in the next hidden layer. The neurons process the input as explained in the paper published by Catherine Higham and Desmond Higham [@DeepLearning].
...
One of the biggest advantages of gradient descent is that one is not constrained with curse of dimensionality. However the algorithm can be stuck in a saddle point. In addition, one must know how to calculate $\nabla Cost(p_t)$ and a good choice of $\eta$ is often required to avoid divergence of iterations. One can see in the Simulation section that our choice of $\eta$ was 0.9, however finding the optimal learning rate is beyond our scope, however the article[@LearningRate] explained that "Generally, a large learning rate allows the model to learn faster, at the cost of arriving on a sub-optimal final set of weights. A smaller learning rate may allow the model to learn a more optimal or even globally optimal set of weights but may take significantly longer to train.".

The calculation of the derivatives of the lost function with respect to weights and biases is not trivial. The authors of the paper[@DeepLearning] have provided the solutions:
...
In logistic regression, the probability of the target variable $Y$ being in a particular class is modeled as a logistic function of a linear combination of the explanatory variables $(X_1,X_2,...,X_n)$. Our simulated dataset only includes $(X_1,X_2)$. 
...
The logistic regression model is implemented using R's 'glm' function with a binomial family. The function estimates the coefficients of the model using maximum likelihood estimation. We train the model on the training set, which includes 800 observations.
...
From the plot comparing the predict data points with the original data points, we can see that due to the linear nature of the logistic regression model, it is inadequate for capturing the circular patterns of the data.

```{r}
test<-NeuralNet$new(data=datos[1:800,], hidden_neurons=40, num_iteration=40000, learning_rate=0.95)
plot(test$output$cost_hist)
```

```{r}
y_pred<-test$makePrediction(testdata=datos[801:1200,], 40)

df_pred<-data.frame(x1=datos[801:1200,"x1"], x2=datos[801:1200,"x2"], y_pred=as.numeric(y_pred), y_true=datos[801:1200,"y"])%>%
  gather("Type", "Val", 3:4)
...
To better describe the performance of the model classification, we used the confusion matrix. The confusion matrix cab ve represented as:
$\begin{bmatrix}TrueNegative & FalsePositive\\ FalseNegative & TruePositive\end{bmatrix}$
...
With the confusion matrix, we can exam the following measures of classificaiton:
$Accuracy=\frac{TP+TN}{TP+TN+FP+FN}$
$Recall=\frac{TP}{TP+FN}$
$Precision=\frac{TP}{TP+FP}$
$F1\ Score=2\times\frac{Precision\times Recall}{Precision+Recall}$

We choose to compare the Accuracy and the F1-Score of both logistic regression model and NN model. The accuracy measures the overall correctness of the model, the proportion of total true predictions to the total number of cases. The F1-Score balance the trade-off between precision and recall.
```{r}
...
The table shows that the Neural Network model substantially outperforms the Logistic Regression model on our simulated dataset. The higher values in both accuracy and F1 Score for the Neural Network suggest that it is more capable of handling the complexities of the dataset, particularly its non-linear patterns.