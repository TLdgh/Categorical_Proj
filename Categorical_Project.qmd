...
The calculation of the derivatives of the lost function with respect to weights and biases is not trivial. One often has to work on cumbersome vector calculus[@MatrixCalc]. The authors of the paper[@DeepLearning] have provided the solutions:
...
We use a simple mechanism to generate nonlinear categorical data based on two covariates [@Data]. Based on the mathematical expressions explained in Section Method, we built our algorithm based on an existing template provided by [@Algo]. 
...
plot_ly(y=test$output$cost_hist, type = "scatter", mode="lines",line = list(width = 1))%>%
  layout(title="Loss History of Feedforward Neural Network",
